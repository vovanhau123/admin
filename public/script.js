...
function createButton(script, status) {
  ...
  startButton.innerHTML = `
    ...
    `;
  startButton.onclick = status.isRunning
    ? null
    : () => startScript(script, startButton);
...
const logsOrder = ["role", "ticket", "toxic"];
...
function updateButton(scriptName, isRunning) {
  const card = document.querySelector(
    `.script-card[data-script="${scriptName}"]`
  );
  if (card) {
    ...
    if (startButton) {
      ...
      startButton.onclick = isRunning
        ? null
        : () => startScript(scriptName, startButton);
    }
  }
}
